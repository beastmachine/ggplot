...
//    System.out.println("pixels per point "+pixelsPerPointY);
    double textWidth = format.getLineWidth(g, this.content, pixelsPerPointX);
    double hJust = format.getHorizontalJustification();
    System.out.println("text height "+format.getLineHeight(g, pixelsPerPointY));
    if(hJust == 0.5){
      g.drawString(this.content, (float)(hJust*pixels.getWidth()-textWidth/2.0), (float)format.getLineHeight(g, pixelsPerPointY));
    }
    else{
  ...
  public Dimension2D getRequiredPointsSize(Graphics2D g) {
    return new GDimension2D(format.getLineWidth(g, this.content, 1),format.getLineHeight(g,1));
  }
